...
class PRISMOrchestrator:
    def __init__(self):
        # Configure Bedrock model for all agents - using inference profile
...
Extract and analyze the Terraform configurations found in issue descriptions for testing."""
    ...
    def _create_terraform_agent(self):
        ...
        return Agent(
            model=self.bedrock_model,
            tools=[self._create_terraform_file, self.terraform_mcp_client],
            system_prompt="""You are a Terraform specialist. Your job is to:
1. Create Terraform test environments with proper configurations
2. Use Terraform MCP tools for advanced operations (init, validate, plan, apply, destroy)
3. Use execute_bash tool for additional terraform commands when needed
4. Analyze terraform outputs and errors
5. Clean up test environments and AWS resources
...
Always work in the appropriate test directory for each issue."""
        )
    ...
    def _create_analysis_agent(self):
        ...
        return Agent(
            model=self.bedrock_model,
            tools=[self._analyze_terraform_output, self._write_triage_report, self.aws_docs_mcp_client],
...
2. For each issue found, delegate to Terraform Agent to test configurations
